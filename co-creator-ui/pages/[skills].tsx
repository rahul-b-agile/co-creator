...
const Navigate = ({ data }: any) => {
    ...
    if (router.query.skills) {
        ...
        useEffect(() => setProfile(data[0].props1.data1.data.filter((data: any, i: number) => (
            data.attributes.primaryskill.toUpperCase() == skill.toUpperCase()
        ))), [])

        useEffect(() => setResult(data[1].data.filter((data: any, i: any) => (
                            ...
                            <div className='col-span-9  '>
                                <div className='ml-[5%]'>
                                    <div className='grid grid-flow-col w-[60%]  m-auto'>
                                        {loading && profile.length >= 1 ? <Cards profile={profile} func={handleProfile}></Cards> : "No Matched Skills"}
