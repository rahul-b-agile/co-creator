...
const Head = ({ result }: any) => {
  ...
  const handleChange = (data: any) => {
    ...
    router.push(`/frontPage/${result[0].attributes.skill}`)
      ...
      <>
        <div className="grid grid-cols-3 gap-4">
          <div className="col-span-2 ml-[20%] w-[70%] ">
            <div className="grid  text-5xl tracking-wide justify-items-center font-semibold">
              {' '}
              Hire the Top {result[0].attributes.skill} Developers in any time
              Zone!
            </div>
            <div className="tracking-wide text-xl w-[90%] font-semibold mt-5 ">
              {result[0].attributes.description}
            </div>
            <div>
              <button
                onClick={() => handleChange(result[0].attributes.skill)}
                className="bg-blue-500 w-[69%] mt-5 tracking-wide ml-[4%]  text-2xl hover:bg-blue-700 text-white  font-bold py-2 px-4 "
              >
          ...
          <div className="col-span-1">
            <div className="grid justify-items-start">
              <img
                width={'50%'}
                height={'60%'}
                src={result[0].attributes.image.data[0].attributes.url}
              ></img>
            </div>
          </div>

          <div className="col-span-3 font-semibold  ml-[14%] text-3xl mt-5 ">
            Get to Hire the ready to deploy trained and experienced{' '}
            {result[0].attributes.skill} developers.
          </div>
        </div>
